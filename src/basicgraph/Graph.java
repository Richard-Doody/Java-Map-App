...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> tempOut = new ArrayList<Integer>();
		List<Integer> degreeSeq = new ArrayList<Integer>();
		for (int i=0; i < numVertices; i++) {
			
			tempIn.addAll(getInNeighbors(i));
		
			tempOut.addAll(getNeighbors(i));
		}
			List<Integer> tempTotal = new ArrayList<Integer>();
			tempTotal.addAll(tempIn);
			tempTotal.addAll(tempOut);
			Collections.sort(tempTotal);
			int count = 0;
			int count2 = 0;
			int temp = 0;
			while(count != tempTotal.size()) {
			count = tempTotal.lastIndexOf(tempTotal.get(count)) + 1;
			temp = count - count2;
			degreeSeq.add(temp);
			count2 = count;
			}
			/*while(tempTotal.size()!=0) {
				
				count = tempTotal.lastIndexOf(tempTotal.get(0)) + 1;
				for (int j = 0; j<count; j++) {
					tempTotal.remove(j);
				}
					/*if(tempTotal.get(0)==tempTotal.get(i)) {
						count++;
					}
					else {
						((ArrayList)tempTotal).removeRange(0,count);
						degreeSeq.add(count);
				}
				*/
