...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		// setup to begin BFS
		HashMap<MapNode,MapNode> parentMap = new HashMap<MapNode,MapNode>();
		Comparator<MapNode> comparator = new RoadLengthComparatorDijkstra();
		PriorityQueue<MapNode>  toExplore = new PriorityQueue<MapNode>(10, comparator);
		
		HashSet<MapNode> visited = new HashSet<MapNode>();
		//set the start nodes distance to zero
		startNode.computeCurrentDistance(0, 0);
		toExplore.add(startNode);
		MapNode next = null;
		...
		while (!toExplore.isEmpty()) {
			next = toExplore.remove();
			countdijk++;
			System.out.println("Count is " +countdijk + " Dijkstra visiting node at location " + next);
			 // hook for visualization
			nodeSearched.accept(next.getLocation());
			...
			if(!visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						if(next.getCurrentDistance()+ next.distance(neighbor)<neighbor.getCurrentDistance()) {
							neighbor.computeCurrentDistance(next.getCurrentDistance(), next.distance(neighbor));
							parentMap.put(neighbor, next);
							toExplore.add(neighbor);
						}
					}
		...
		List<GeographicPoint> path =
				reconstructPath(parentMap, startNode, endNode);
		System.out.println("Dijkstra count is " + countdijk);
		return path;
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		// setup to begin BFS
		HashMap<MapNode,MapNode> parentMap = new HashMap<MapNode,MapNode>();
		Comparator<MapNode> comparator = new RoadLengthComparator();
		PriorityQueue<MapNode>  toExplore = new PriorityQueue<MapNode>(10, comparator);
		
		HashSet<MapNode> visited = new HashSet<MapNode>();
		//set the start nodes distance to zero
		startNode.computeCurrentDistance(0, 0);
		startNode.setCombinedDistance(0);
		toExplore.add(startNode);
		MapNode next = null;
		...
		while (!toExplore.isEmpty()) {
			next = toExplore.remove();
			countAstar++;
			System.out.println("Astar Count is " + countAstar + " Astar visiting node at location" + next);
			 // hook for visualization
			nodeSearched.accept(next.getLocation());
			...
			if(!visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						if(next.getCurrentDistance()+ next.distance(neighbor) + neighbor.distance(endNode)<neighbor.getCombinedDistance()) {
							neighbor.setCombinedDistance(next.getCurrentDistance()+ next.distance(neighbor)+ neighbor.distance(endNode));
							neighbor.computeCurrentDistance(next.getCurrentDistance(), next.distance(neighbor));
							//System.out.println(neighbor.getCombinedDistance());
							parentMap.put(neighbor, next);
							toExplore.add(neighbor);
						}
					}
		...
		*/
		/*
		  MapGraph simpleTestMap = new MapGraph();
			...
			MapGraph testMap1 = new MapGraph();
			GraphLoader.loadRoadMap("data/maps/utc.map", testMap1);
			// A slightly more complex test using real data
			testStart = new GeographicPoint(32.8674388, -117.2190213);
			testEnd = new GeographicPoint(32.8697828, -117.2244506);
			System.out.println("Test 3 using utc: Dijkstra should be 37 and AStar should be 10");
			testroute = testMap1.dijkstra(testStart,testEnd);
			testroute2 = testMap1.aStarSearch(testStart,testEnd);*/
